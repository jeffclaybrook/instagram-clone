...
const Comments = ({ postId, comments, user }: { postId: string; comments: CommentWithExtras[]; user?: User | null }) => {
  ...
  <div className="space-y-0.5 px-3 sm:px-0">
   ...
   <Form {...form}>
    ...
    >
     ...
     />
     {body.trim().length > 0 && (
      <button type="submit" className="text-sky-500 text-sm font-semibold hover:text-white disabled:hover:text-sky-500 disabled:cursor-not-allowed">Post</button>
     )}
    </form>
   </Form>
  </div>
